...
class ClovaOCRAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="이미지 파일과 자녀 이름을 받아 OCR을 수행하고, 결과를 데이터베이스에 저장합니다.",
        manual_parameters=[
            openapi.Parameter(
                "image",
                openapi.IN_FORM,
                description="OCR을 수행할 이미지 파일",
        ...
        responses={
            201: openapi.Response(description="처방전이 성공적으로 등록되었습니다."),
            400: openapi.Response(description="처방전 저장 중 오류가 발생했습니다."),
            500: openapi.Response(
                description="Clova OCR API 호출 중 오류가 발생했습니다."
            ),
        },
    )
    @transaction.atomic
    def post(self, request, *args, **kwargs):
        if "image" not in request.data or "child_name" not in request.data:
            ...
            return Response(
                {"error": "No image file or child name provided"}, status=400
            )
        ...
        try:
            response = requests.post(
                OCR_API_URL, headers=headers, data=json.dumps(payload)
            )
            response.raise_for_status()
        except requests.RequestException as e:
            logger.exception("Error calling Clova OCR API")
            return Response(
                {"error": "Error calling Clova OCR API", "detail": str(e)}, status=500
            )
        ...
        try:
            ...
            return Response(
                {"error": "Invalid JSON response from Clova OCR API"}, status=500
            )
        ...
        try:
            ...
            return Response(
                {"error": f"처방전 저장 중 오류가 발생했습니다: {str(e)}"}, status=400
            )
